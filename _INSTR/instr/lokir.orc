...
gSlokir_path init "/Users/j/Documents/PROJECTs/CORDELIA/_INSTR/sonvs/samps-loki"
gilokir_mecha	init 1
	...
	Spath_lateral			sprintf "%s/FP_%s_%s_%s_%s.wav", gSlokir_path, Smics_lateral[(floor(icps) % 2)], Snote_name, Sdyn, S_RR_lateral
	...
	Spath_central			sprintf "%s/FP_%s_%s_%s_%s.wav", gSlokir_path, "C", Snote_name, Sdyn, S_RR_central
	...
	am						= .5+oscil3:a(cosseg:a(.5, idur_am, 0), icps_am)
	aout					= aout * am
	;prints sprintf("%s\n", Spath_lateral)
	...
	;================================================================
	if idur < 7.5 then
		Spath_release sprintf "%s/KeyNoise_RR%i.wav", gSlokir_path, floor(index_RR_lateral / imax_RR)*15
		schedule "lokir_release", 0, 5, Spath_release, idyn/3, ich
	endif
	...
	if  idyn < ampdbfs(-7) then
		Spath_release sprintf "%s/release_soft_RR%i.wav", gSlokir_path, index_RR_release
	else
		Spath_release sprintf "%s/release_hard_RR%i.wav", gSlokir_path, index_RR_release
	endif
	schedule "lokir_release", idur, idur, Spath_release, idyn*gilokir_mecha, ich
	;================================================================
