...
instr orphans3xs_instr
	...
	if (itie == 0 && p3 < 0) then
		...
		aenv_vib	= table3:a(linseg:a(0, iatk, indx), ienv)*ampdb(-11)
		kcps	init icps

	elseif (p3 < 0 && itie == 1) then
		;prints "TIED: MIDDLE NOTE\n"
		aenv		cosseg idyn_last, iport, idyn
		aenv_vib	cosseg ampdb(-11), iport, ampdb(-13), iport, ampdb(-11)
	...
	if krel == 1 then
		...
		aenv_vib	= table3:a(linseg:a(indx+1, irel_time, gienvdur), ienv)*cosseg(ampdb(-11), iport, ampdb(-13), iport, ampdb(-11))
		kcps	cosseg icps_last, irel_time, icps_rel

	endif

	a1		oscil3 1, kcps + lfo:a(icps_var, icps_var/int(random:i(1, 5))), ift1, iphase
	a1		*= .75 + oscil3:a(.25, 2*aenv_vib, gisine, iphase)

	a2		oscil3 1, kcps + lfo:a(icps_var, icps_var/int(random:i(1, 5))), ift1, iphase
	a2		*= .5 + oscil3:a(.5, 3*aenv_vib * 2, gisine, iphase)

	a3		oscil3 1, kcps + lfo:a(icps_var, icps_var/int(random:i(1, 5))), ift2, iphase
	a3		*= .75 + oscil3:a(.25, 3*aenv_vib * 3, gisine, iphase)

	;a3			chebyshevpoly  a3, k6, k5, k4, k3, k2, k1, k0
	kcps_moog = limit(15$k - (kcps * (1-aenv) * 15), 20, 20$k)
	a3		moogladder2 a3, kcps_moog, .5+oscil3:a(.15, 3*aenv_vib, gisine, iphase), itie
