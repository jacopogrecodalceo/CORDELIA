...
def extract_if_statements(xml_string):
	...
	for i in range(len(note_names_extracted)-1):
		note_name, note_num = note_names_extracted[i]
		next_note_name, next_note_num = note_names_extracted[i + 1]
	...
	Snote_name init "{note_name}"'''
			...
			continue
		elif i > 0 and i < len(note_names_extracted)-1:
			lo_note = note_num-1
			hi_note = next_note_num-1
			root_note = note_num	
		elif i == len(note_names_extracted)-1:
			lo_note = note_num-1
			hi_note = 127
			root_note = note_num
...
path = '/Users/j/Documents/PROJECTs/CORDELIA/_INSTR/sonvs/samps-echorda2/_ECHORDA_3.dspreset'
