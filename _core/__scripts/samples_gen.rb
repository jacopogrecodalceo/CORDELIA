default_samp      = File.open('../__instr/_sample/_sampleDEF.txt').read
default_dir       = File.open('../__instr/_sample/_dirDEF.txt').read

samp_dir           = '../../samples/opcode/*' 
samp_wav_dir       = '../../samples/opcode/*.wav'
samp_orc_dir       = '../__instr/_sample'
samp_orc_each      = samp_orc_dir + '/*.orc'


#DELETE ALL .ORC
Dir[samp_orc_each].each do |file|
    File.delete(file)
end


#GENERATE FROM SAMPLES
Dir[samp_wav_dir].each do |path|
    ...
    default_samp.each_line do |line|
        begin
