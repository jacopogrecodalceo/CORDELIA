...
function remove_before_cursor(item)
...
function on_play()
	if STATE then
		...
		for index, item in pairs(items) do
			...
			remove_before_cursor(item)
			
			if #item == 7 then --note midi
				table.insert(midi_items, item)
...
function cordelia_realtime(play_pos)
	...
	if reaper.GetPlayState() == 1 then
		...
		for i, item in ipairs(midi_items) do
		...
		for i, item in ipairs(text_items) do
			...
			if start_pos <= play_pos and not contains_value(items_turnoff, item) then
				break
			end
