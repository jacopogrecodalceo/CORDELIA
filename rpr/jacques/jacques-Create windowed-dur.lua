...
GLOBAL_BPM = reaper.TimeMap2_GetDividedBpmAtTime(0, 0)
...
local function main(segment_dur, subdivision) -- local (i, j, item, take, track)
...
local retval, retvals_csv = reaper.GetUserInputs(SCRIPT_NAME, 2, 'Segment duration:,Subdivision:', 60/GLOBAL_BPM .. ',8', 512)
if retval then
	local values = split_csv(retvals_csv)
	local exec_values = {}
	for _, value in pairs(values) do
		local exec_func = load('return ' .. value)
		if exec_func then  -- Check if exec_func is not nil
			local res = exec_func()
			table.insert(exec_values, res)
		else
			reaper.ShowMessageBox('Error loading function from value: ' .. value, 'Error', 0)
		end
	end
	local segment_dur, subdivision = table.unpack(exec_values)

	main(segment_dur, subdivision)
end
--RestoreCursorPos()
--RestoreLoopTimesel()
