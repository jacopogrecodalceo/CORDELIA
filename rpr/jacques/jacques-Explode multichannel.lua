...
if count > 0 then
  ...
  for i, item in ipairs(item_list) do
    ...
    if take then
      ...
      if reaper.GetMediaSourceSampleRate(source) > 0 then
        ...
        local parent_track = reaper.GetMediaItem_Track(item)
          ...
          if track == previous_track then
            if track_max_chan < take_chan then
              for j=0, take_chan - track_max_chan - 1 do
                ...
                reaper.GetSetMediaTrackInfo_String(new_track,"P_NAME",track_name.." - ch"..(j+track_max_chan+1),true)
            ...
            for j=0, take_chan - 1 do
              ...
              table.insert(new_tracks_list,new_track)
              reaper.GetSetMediaTrackInfo_String(new_track,"P_NAME",track_name.."ch"..(j+1),true)

              for p, parname in ipairs(track_parnames) do
                  reaper.SetMediaTrackInfo_Value(new_track,parname,reaper.GetMediaTrackInfo_Value(track,parname))
              end

              reaper.SetMediaTrackInfo_Value(new_track,"I_NCHAN",take_chan)
              reaper.SetMediaTrackInfo_Value(new_track,"C_MAINSEND_NCH",1)
              reaper.SetMediaTrackInfo_Value(new_track,"C_MAINSEND_OFFS",j)

              --reaper.GetSetMediaTrackInfo_String( parent_track, "P_NAME", parentName, true )
              reaper.SetMediaTrackInfo_Value( parent_track, "I_FOLDERDEPTH", 1, true )
  ...
  reaper.Undo_EndBlock("Explode multichannel",0)
  reaper.PreventUIRefresh(-1)
