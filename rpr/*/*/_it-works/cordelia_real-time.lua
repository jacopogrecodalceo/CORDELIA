...
function get_all_midi_attrs(ids, items)
	...
	for note_index = 0, notecnt-1 do
		local retval, selected, muted, startppqpos, endppqpos, note_chn, note_pitch, note_velocity = reaper.MIDI_GetNote(take, note_index)
	...
	for i = 0, reaper.CountTracks(0)-1 do
		...
		for j = 0, reaper.GetTrackNumMediaItems(track)-1 do
			...
			if take ~= nil and reaper.TakeIsMIDI(take)then -- if ==, it will work on "empty"/text items only
				get_all_midi_attrs(ids, items)
			else
				get_all_empty_attrs(ids, items)
			end
		end
	end
end
